...
var Keycloak = class {
    ...
    init = async (initOptions = {}) => {
        ...
        if (
            ...
            ['default', 'cordova', 'cordova-native'].includes(initOptions.adapter)
        )
        ...
        return {
            login: async (options) => {
                ...
                const ref = cordovaOpenWindowWrapper(
                    await this.createLoginUrl(options),
                    '_blank',
                    cordovaOptions,
                );
                let completed = false;
                ...
                return await new Promise((resolve, reject) => {
                    ...
                    ref.addEventListener('exit', function (event) {
                        if (!closed)
                            reject(/* @__PURE__ */ new Error('User closed the login window.'));
                    });
                });
            },
            logout: async (options) => {
                const ref = cordovaOpenWindowWrapper(
                    this.createLogoutUrl(options),
                    '_blank',
            ...
            register: async (options) => {
                const ref = cordovaOpenWindowWrapper(
                    await this.createRegisterUrl(),
                    '_blank',
                    createCordovaOptions(options),
                );
                await new Promise((resolve, reject) => {
                    ref.addEventListener('loadstart', async (event) => {
                        if (event.url.indexOf(getCordovaRedirectUri()) === 0) {
    ...
    async #loadOidcConfig(oidcProvider) {
        if (typeof oidcProvider === 'string') {
            const openIdConfig = await fetchOpenIdConfig(
                `${stripTrailingSlash(oidcProvider)}/.well-known/openid-configuration`,
            );
    ...
    async #check3pCookiesSupported() {
        ...
        return await applyTimeoutToPromise(
            new Promise((resolve) => {
                /**
    ...
    async #setupCheckLoginIframe() {
        ...
        const messageCallback = (event) => {
            ...
            for (const callback of callbacks.reverse())
                if (event.data === 'error')
                    callback(/* @__PURE__ */ new Error('Error while checking login iframe'));
    ...
    createLoginUrl = async (options) => {
        ...
        const url =
            options?.action === 'register' ? this.endpoints.register() : this.endpoints.authorize();
    ...
    createAccountUrl = (options) => {
        ...
        return `${url}/account?${new URLSearchParams([
    ...
    loadUserProfile = async () => {
        ...
        return (this.profile = await fetchJSON(`${realmUrl}/account`, {
     ...
     */
    loadUserInfo = async () => {
        return (this.userInfo = await fetchJSON(this.endpoints.userinfo(), {
            headers: [buildAuthorizationHeader(this.token)],
        }));
    };
    /**
...
async function generatePkceChallenge(pkceMethod, codeVerifier) {
    ...
    return bytesToBase64(new Uint8Array(await sha256Digest(codeVerifier)))
        .replace(/\+/g, '-')
        .replace(/\//g, '_')
        .replace(/=/g, '');
}
...
function applyTimeoutToPromise(promise, timeout, errorMessage) {
    ...
    const timeoutPromise = new Promise(function (resolve, reject) {
        timeoutHandle = window.setTimeout(function () {
            reject(
                new Error(
                    errorMessage || 'Promise is not settled within timeout of ' + timeout + 'ms',
                ),
            );
        }, timeout);
    });
...
//# sourceMappingURL=keycloak.D4BYgFqv.js.map
